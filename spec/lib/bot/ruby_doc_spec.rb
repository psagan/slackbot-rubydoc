RSpec.describe Bot::RubyDoc do
  describe "#handle_data" do
    ...
    it "yields block when given" do
      ...
      expect { |b| bot.handle_data(data, &b) }.to yield_control.once
      expect { |b| bot.handle_data(data, &b) }.to yield_with_args(test_string)
    end
    ...
    it "yields block multiple times when response is longer" do
      ...
      expect { |b| bot.handle_data(data, &b) }.to yield_control.exactly(quantity)
    end
    ...
    it "yields with unknown_command  when ri returns empty string" do
      ...
      expect { |b| bot.handle_data(data, &b) }.to yield_control.once
      expect { |b| bot.handle_data(data, &b) }.to yield_with_args(unknown_command)
    end
  end
end